...
在已测试的 Galaxy 上，应用和恢复期间移动数据都保持可用。SIM 运营商层仍可能在后续重连时
影响 IMS 通话或短信，因此本工具适合短时间、可恢复的使用，而不是永久修改运营商身份。
...
- **是否重启应用由你决定。**应用和恢复都不会自动停止目标应用；需要时再手动强停、清缓存或
  数据并重新打开。
...
关闭某一层的开关，只表示下一次应用时不写这一层，**不会清除已经生效的覆盖**。清除已有覆盖
必须使用恢复。重启手机是核心临时覆盖的最终复位方式。
...
| **两层一起** | 同时对比两类信号的应用 | 先应用国家/地区，再应用 SIM 运营商 | 覆盖信号更完整，但不会消除 SIM 运营商层的 IMS 风险 |

建议先使用范围更窄的一层。TikTok 一类的国家判断通常先试“应用国家/地区”；Galaxy Store 和
三星应用的运营商判断则需要“SIM 运营商”。账号地区、IP、CSC、GPS、应用版本、服务端实验和缓存
也可能参与判断，因此两层都生效也不代表所有应用一定切换。
...
通知权限是可选的。拒绝不会影响应用或恢复，只会失去状态提示、常驻提醒和快捷按钮。

## 要求与测试范围

- Android 10（API 29）或更高版本；
- Shizuku 13+，以 shell 或 root 运行并完成授权；本应用不要求 root；
- 有效订阅；“SIM 运营商”层还要求 SIM 处于 `READY` 状态；
- 主要面向较新的三星固件，其他三星和非三星系统属于实验兼容目标。
...
应用或恢复伪装时不会自动停止它们。**保留**不清理存储，**缓存**请求只清缓存，**数据**会先停止
所选应用，再清除它的全部本地数据。清除数据会退出账号，并可能删除下载、草稿和设置。如果固件不
支持只清缓存或执行超时，结果卡会如实显示，不会假装成功。
...
应用国家/地区；只要假运营商可能仍在，就不会主动切换 UICC。之后再恢复保存的显示名称，并把
无法确认的 IMS 恢复显示为警告，而不是成功。应用后的 IMS 观察只说明当时状态，不能保证下一次
重连仍然正常。
...
- **恢复状态：**第一次写入前保存真实 MCC/MNC、运营商名、国家 ISO 和订阅显示名称；每次 Binder
  写入前同步记录 pending 状态，进程中途消失也不会让应用忘记需要恢复。
...
CarrierConfig 重载等待有明确上限；部分成功时仍保存足够状态，让用户继续恢复，而不是误报“什么
都没发生”。
...
应用没有 Internet 权限、遥测或账号系统，也不会自动读取或上传全局日志。收集流程见
...
- 某些固件不允许读取稳定 card 标识，此时无法把新快照绑定到已验证的卡身份；如果可读性后来发生
  变化，界面会要求采用更保守的重置流程；
