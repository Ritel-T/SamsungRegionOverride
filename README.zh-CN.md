...
项目以较新的三星手机为主要支持目标，已在 **SM-S938B、Android 16 / One UI 8.5** 和 **Android 17 / One UI 9 Beta** 上测试。其他三星或非三星设备属于兼容目标。
...
应用只读取承载移动数据的那一张 SIM。在双卡机上，把伪装写到闲置的那张卡会正常应用、报告成功，但任何应用都看不到变化——覆盖确实生效了，本工具读回的每一个字段也都同意它生效了。SIM 选择器会给承载数据的那张卡标上 **DATA**，并在所选 SIM 不是它时明确提示。切换哪张卡用数据要在系统设置里改，本工具不负责。

应用地区不一定只由 SIM 决定。账号地区、IP、CSC、应用版本、服务端实验和缓存都可能参与判断。修改地区后，如果目标应用仍使用旧缓存，可以在“目标应用”面板中手动停止并重新启动它。
...
结果卡默认保持简洁，需要排查时再展开查看本地操作详情或安全的 `SRO-DIAGNOSTIC/1` 摘要。复制、分享和提交问题只使用该摘要，不会自动上传完整 logcat 或手机/SIM 标识。详见[诊断说明](docs/diagnostics.md)。
...
目标列表可以自行修改。面板支持手动强制停止、清缓存/清数据和重新启动；开启或恢复伪装不会自动停止目标应用。清除 **Data** 会退出账号，并删除下载、草稿与本地设置。测试固件上的 cache-only 清除可能超时或无效果，报告会如实显示。
...
- 目前测试范围仅覆盖 SM-S938B 的 Android 16 / One UI 8.5 和 Android 17 / One UI 9 Beta；
- 当前界面最多展示两个活动订阅；
- 预设只是便捷数据，不是实时运营商数据库；
